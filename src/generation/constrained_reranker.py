...
def build_constrained_prompt_manifests(
    source: Path, confirmation_source: Path, output_dir: Path
) -> dict[str, Any]:
    ...
    payload = {
        "schema_version": 1,
        "experiment": CONSTRAINED_EXPERIMENT,
...
def load_selector_config(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("experiment") != CONSTRAINED_EXPERIMENT
...
def select_constrained_candidates(
    *,
    ...
    if any(len(pool) != 8 for pool in pools.values()):
        raise LTSNContractError("constrained selection requires complete best-of-8 pools")
    baseline = {
...
def apply_constrained_selection(
    *,
    ...
    run_root = experiment_root(project_root, config)
    load_selector_config(selector_config_path)
...
def freeze_calibrated_selector(
    *,
    ...
    payload = {
        ...
        "constraints": load_selector_config(selector_config_path)["constraints"],
...
def load_selection_contract(
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if (
        payload.get("experiment") != CONSTRAINED_EXPERIMENT
