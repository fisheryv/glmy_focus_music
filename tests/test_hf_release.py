...
from data.hf_release import DatasetReleaseError, prepare_release_dataset
...
def test_prepare_release_dataset_verifies_and_materializes(tmp_path: Path) -> None:
    ...
    _write_csv(
        ...
        [{"track_id": "one", "relative_path": "focus_music/one.mp3", "sha256": digest}],
    )
...
def test_prepare_release_dataset_rejects_tampered_audio(tmp_path: Path) -> None:
    ...
    _write_csv(
        project_metadata / "track_index.csv",
        [{"track_id": "one", "relative_path": "focus_music/one.mp3", "sha256": expected}],
    )
