...
from repetition.analysis import PATH_REPRESENTATIONS, load_config
...
ANALYSIS_SUMMARY_PATH = METADATA / "phase_lifted_path_homology_summary.json"
REPORT_SUMMARY_PATH = METADATA / "phase_lifted_path_homology_report_summary.json"
    ...
    *,
    ...
    lines = [
        ...
        *_markdown_table(
        ...
        "- `scripts/render_phase_lifted_report.py`",
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError(
            "phase-lifted analysis outputs are missing; run "
            "scripts/run_phase_lifted_analysis.py first: "
            + ", ".join(missing)
        )
    ...
    figure_paths.extend(_plot_representative_cycles(representatives))
    _write_report(
        excluded=excluded,
        features=features,
    ...
    artifacts = [REPORT_PATH, *figure_paths]
    payload = {
        "generated_at": date.today().isoformat(),
        "scope": "phase-lifted Path Homology report rendering",
        "ok": True,
        "input_sha256": {
    ...
    _write_json_atomic(REPORT_SUMMARY_PATH, payload)
