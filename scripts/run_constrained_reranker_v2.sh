...
PIPELINE_ROOT="${PIPELINE_ROOT:-runs/topology_rerank_lora_v1/constrained_v2}"
SELECTOR_CONFIG="${SELECTOR_CONFIG:-configs/constrained_reranker_v2.json}"
CALIBRATION_CONFIG="${CALIBRATION_CONFIG:-configs/topology_constrained_calibration_v2.toml}"
CONFIRMATION_CONFIG="${CONFIRMATION_CONFIG:-configs/topology_constrained_confirmation_v2.toml}"
FROZEN_SELECTOR="${PROJECT_ROOT}/${PIPELINE_ROOT}/frozen_selector.json"
GATE_OUTPUT="${GATE_OUTPUT:-${PROJECT_ROOT}/metadata/ace_constrained_reranking_effect_gate_v2.json}"
STAGE="${1:-}"
...
CAL_RUN="${PROJECT_ROOT}/${PIPELINE_ROOT}/rerank/topology_constrained_calibration_v2"
CONF_RUN="${PROJECT_ROOT}/${PIPELINE_ROOT}/rerank/topology_constrained_confirmation_v2"
...
case "${STAGE}" in
  prepare-prompts)
    "${PYTHON_BIN}" "${PROJECT_ROOT}/scripts/run_constrained_reranker.py" prepare-prompts \
      ...
      --prompt-output "${PIPELINE_ROOT}/prompts"
    ;;
  calibration-run)
  ...
  confirmation-run)
    if [[ ! -f "${FROZEN_SELECTOR}" ]]; then
      echo "A supported frozen v2 calibration selector is required." >&2
      exit 2
    fi
