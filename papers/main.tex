...
音高、节奏和调制视角分别描述和声位置、节奏组织与幅度调制轮廓。为检验这些信息是否互补，本文将三组各 20 个预设拓扑描述子作为独立块进行后融合。整体流程如图~\ref{fig:local_multiview_fusion} 所示。
...
根据前述单指标验证、多尺度稳定性与融合增量分析，本文将 Focus Music 的拓扑指纹
冻结为三维 Pitch Path Homology 表示，记为
\begin{equation}
\Phi_{\mathrm{PH}}(x)=
...
\end{equation}
该指纹只保留局部 Pitch 分支，因为跨周期相位及其与 Pitch 的联合表示均未稳定优于
最佳 Pitch 单块，而 Pitch 是局部融合中唯一具有稳定正向几何增量的视角。三个坐标
分别描述局部连通与状态覆盖、局部稳定与停留以及有向重复结构，在统计支持、跨时长
稳定性和结构含义之间形成互补。

\begin{table}[htbp]
\centering
\caption{Focus Music 三维拓扑指纹的冻结组成与验证依据}
...
与直接采用 \texttt{vertex\_count} 和 \texttt{edge\_count} 相比，当前组合避免两个
图规模计数特征重复表达相近信息，并保留了状态覆盖、停留和有向复现三种不同机制。
三项指标均在预设 Pitch 检验族中通过 validation/180s 的 BH-FDR，并在同曲
validation/300s 保持方向和较高配对 Spearman 相关。三维辅助逻辑回归在
validation/180s 的 balanced accuracy 为 $0.942$、AUROC 为 $0.980$，在 300s
敏感性分析中的对应值为 $0.942$ 和 $0.988$；这些结果表明该低维表示保留了主要组间
判别信息，但不等于生成可控性或注意力改善证据。
...
\begin{enumerate}
  ...
  \item validation/180s 用于主要验证，validation/300s 仅作同曲时长敏感性；
  已开启 holdout 不用于变换拟合、目标带设定、权重或阈值调整；
  \item 指纹 JSON、逐曲分数、配置、代码和来源表均以 SHA-256 绑定。任何特征替换、
  顺序变化、重新加权或目标带修改都必须建立新版本。
\end{enumerate}

该三维对象是本文后续精确教师、代理训练和生成控制共同使用的唯一冻结拓扑指纹。
冻结后，不允许替换指标、改变坐标顺序、重新加权或根据生成结果修改目标带；任何此类
变化都必须建立新的指纹版本并重新完成验证。该指纹是可审计的结构表示，不是注意力、
疗效或音乐质量的因果量表，也不凭自身授权采样期校正；只有代理通过独立资格检验后，
corrector 才能由关闭状态转为候选启用。
